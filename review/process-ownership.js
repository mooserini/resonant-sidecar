...
export function verifyOwnershipTopology(evidence, policy) {
  ...
  record(safe, ['schemaVersion', 'status', 'macOSVersion', 'architecture', 'bootSessionUUID', 'processes', 'sessionFileIdentity', 'checks', 'passed']);
  ...
  for (const process of safe.processes) {
    record(process, ['pid', 'name', 'present', 'ppid', 'pgid', 'startTime', 'elapsedTime', 'executablePath', 'executableDigest', 'cwd', 'signing', 'descriptors', 'endpoints', 'listeners', 'checks', 'sampleDigest']);
    ...
    if (process.listeners !== undefined) {
      if (!Array.isArray(process.listeners)) fail();
      for (const listener of process.listeners) {
        record(listener, ['fd', 'protocol', 'address', 'port', 'transport']);
        if (listener.protocol !== 'TCP' || typeof listener.address !== 'string' || !Number.isInteger(listener.port) || listener.port < 0 || listener.port > 65535 || !['tcp', 'chrome-loopback-cdp'].includes(listener.transport)) fail();
      }
    }
